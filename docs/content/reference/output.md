...
describes what users get. Pick a format with `-o`, or let greynoise choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
greynoise <command> -o table   # aligned columns for reading
greynoise <command> -o jsonl   # one JSON object per line, for piping
greynoise <command> -o json    # a single JSON array
greynoise <command> -o csv     # spreadsheet friendly
greynoise <command> -o tsv     # tab-separated
greynoise <command> -o url     # just the URL column
greynoise <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
greynoise <command> --fields id,title,url
```
...
```bash
greynoise <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
greynoise <command>            # a table, because this is a terminal
greynoise <command> | wc -l    # JSONL, because this is a pipe
```
